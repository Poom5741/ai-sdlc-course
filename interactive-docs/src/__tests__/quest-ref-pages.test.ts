...
describe('Quest MDX content files', () => {
  ...
  for (const file of quests) {
    it(`${file} exists in content/docs/en/quests/`, () => {
      expect(existsSync(join(DOCS_DIR, 'en', 'quests', file))).toBe(true);
    });

    it(`${file} has valid frontmatter with title`, () => {
      const content = readFileSync(join(DOCS_DIR, 'en', 'quests', file), 'utf-8');
...
describe('Reference MDX content files', () => {
  ...
  for (const file of refs) {
    it(`${file} exists in content/docs/en/reference/`, () => {
      expect(existsSync(join(DOCS_DIR, 'en', 'reference', file))).toBe(true);
    });

    it(`${file} uses Starlight component imports`, () => {
      const content = readFileSync(join(DOCS_DIR, 'en', 'reference', file), 'utf-8');
